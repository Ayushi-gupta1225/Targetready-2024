...
class BinarySearchTree {
    ...
    fun displayAllLeafNodes() {
        println("Leaf nodes in the BST: ")
        displayLeafNodes(root)
        println()
    }

    private fun displayLeafNodes(node: TreeNode?) {
        if (node != null) {
    ...
    fun displayNodesAtGivenLevel(level: Int) {
        println("Nodes at level $level: ")
        displayNodesAtLevel(root, level)
        println()
    }

    private fun displayNodesAtLevel(node: TreeNode?, level: Int) {
        if (node != null) {
            if (level == 0) {
                print("${node.key}, ")
            } else {
                displayNodesAtLevel(node.left, level - 1)
                displayNodesAtLevel(node.right, level - 1)
            }
        }
    }

    fun deleteSubtreeAtNode(nodeKey: Int) {
        root = deleteSubtree(root, nodeKey)
    }

    private fun deleteSubtree(node: TreeNode?, key: Int): TreeNode? {
        if (node == null) return null

        if (key < node.key) {
            node.left = deleteSubtree(node.left, key)
        } else if (key > node.key) {
            node.right = deleteSubtree(node.right, key)
        } else {
            node.left = null
            node.right = null
        }
...
fun main() {
    ...
    do {
        ...
        when (choice) {
            ...
            12 -> bst.displayAllLeafNodes()
            13 -> {
                val level = readInt("Enter the level to display nodes: ")
                bst.displayNodesAtGivenLevel(level)
            }
            14 -> {
                val nodeKey = readInt("Enter the key of the node whose subtree to delete: ")
                bst.deleteSubtreeAtNode(nodeKey)
            }
        }
